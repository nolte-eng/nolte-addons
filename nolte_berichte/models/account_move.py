...
class AccountMove(models.Model):
    ...
    def _compute_l10n_german_document_title(self):
        for record in self:
            record.l10n_german_document_title = ''
            if record.move_type == 'out_invoice':
                if record.state == 'posted':
                    record.l10n_german_document_title = _('Invoice')
                elif record.state == 'draft':
                    record.l10n_german_document_title = _('Draft Invoice')
                elif record.state == 'cancel':
                    record.l10n_german_document_title = _('Cancelled Invoice')
            elif record.move_type == 'out_refund':
                record.l10n_german_document_title = _('Credit Note')
            elif record.move_type == 'in_refund':
                record.l10n_german_document_title = _('Vendor Credit Note')
            elif record.move_type == 'in_invoice':
                record.l10n_german_document_title = _('Vendor Bill')
